...
Run `npm run build:prompts` after adding or editing any file under `src/common/prompts/`.

### Adding content is data-only

| What to add        | Where                                                              | Command                 |
| ------------------ | ------------------------------------------------------------------ | ----------------------- |
| New prompt         | Add `*.prompt.ts` to its category `index.ts` barrel                | `npm run build:prompts` |
| New skill          | Add `src/common/prompts/skills/<slug>/skill.ts`                    | `npm run build:prompts` |
| New model          | Add entry to `src/common/prompts/registries/models.ts`             | `npm run build:prompts` |
| New style / tone   | Add entry to `registries/styles.ts` or `registries/tones.ts`       | `npm run build:prompts` |
| New context preset | Add entry to `registries/contexts.ts` with `styleId`/`toneId` refs | `npm run build:prompts` |

**Generated files are git-ignored** — never commit `manifest.generated.ts` or `loaders.generated.ts`.
...
src/
  ...
  common/
    prompts/
      catalog/              # TypeScript prompt modules (source of truth); one *.prompt.ts per logical prompt
        a-software-engineering/
        b-writing-communication/
        c-thinking-productivity/
        d-ai-prompt-workflows/
        index.ts            # top-level barrel
      skills/               # TypeScript skill modules; one <slug>/skill.ts + files/ per skill
      registries/           # models.ts, styles.ts, tones.ts, contexts.ts, value-sets.ts, abbreviations.ts
      generated/            # .gitkeep placeholder; manifest.generated.ts + loaders.generated.ts are regenerated by build:prompts (git-ignored)
      loader.ts             # loadManifest · loadCategory · loadSkill — runtime data accessors
