...
describe('convertUnit — temperature', () => {
    it('0°C = 32°F', () => {
        ...
        expect(Number.parseFloat(f!.value)).toBeCloseTo(32);
    });

    it('100°C = 212°F', () => {
        const results = convertUnit(100, 'c', 'temperature');
        const f = results.find((r) => r.id === 'f');
        expect(Number.parseFloat(f!.value)).toBeCloseTo(212);
    });

    it('0°C = 273.15 K', () => {
        const results = convertUnit(0, 'c', 'temperature');
        const k = results.find((r) => r.id === 'k');
        expect(Number.parseFloat(k!.value)).toBeCloseTo(273.15);
    });

    it('-40°C = -40°F (crossover point)', () => {
        const results = convertUnit(-40, 'c', 'temperature');
        const f = results.find((r) => r.id === 'f');
        expect(Number.parseFloat(f!.value)).toBeCloseTo(-40);
    });
});
...
describe('convertUnit — time', () => {
    it('1 hour = 3600 seconds', () => {
        ...
        expect(Number.parseFloat(s!.value)).toBeCloseTo(3600);
    });

    it('1 week = 7 days', () => {
        const results = convertUnit(1, 'week', 'time');
        const day = results.find((r) => r.id === 'day');
        expect(Number.parseFloat(day!.value)).toBeCloseTo(7);
    });
});
...
describe('convertUnit — length', () => {
    it('1 km = 1000 m', () => {
        ...
        expect(Number.parseFloat(m!.value)).toBeCloseTo(1000);
    });

    it('1 inch = 2.54 cm', () => {
        const results = convertUnit(1, 'in', 'length');
        const cm = results.find((r) => r.id === 'cm');
        expect(Number.parseFloat(cm!.value)).toBeCloseTo(2.54);
    });
});
...
describe('convertUnit — edge cases', () => {
    it('NaN input produces NaN values', () => {
        const results = convertUnit(Number.NaN, 'c', 'temperature');
