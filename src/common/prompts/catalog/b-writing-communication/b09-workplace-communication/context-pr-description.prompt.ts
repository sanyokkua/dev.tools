...
export const prompt: LogicalPromptDef = {
    ...
    variantAxes: [],
    defaultVariantId: 'USR-B-context-pr-description',
    modeClass: 'chat-only',
    variants: [
        {
            id: 'USR-B-context-pr-description',
            kind: 'user',
            categoryCode: 'B09',
            title: 'Write a Pull Request (PR) Description',
            description: 'Write a Pull Request (PR) Description',
...
Return ONLY the finished message in {{user_format}}, with no commentary. If there is no processable text, return \`[NO_TEXT_PROVIDED]\`.`,
            parameters: [
                {
                    ...
                    control: 'textarea',
                    optional: false,
                    label: 'What the change does (rough notes)',
                },
                {
                    name: 'context',
                    control: 'select',
            ...
            examples: {
                user_text: [
                    'added retry logic to the payment webhook because it dropped events on timeout; tested with a forced timeout locally',
                ],
            },
            keywords: ['pull request', 'PR description', 'what why testing', 'context'],
