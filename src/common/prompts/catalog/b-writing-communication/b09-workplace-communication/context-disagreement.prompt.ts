...
export const prompt: LogicalPromptDef = {
    ...
    variantAxes: [],
    defaultVariantId: 'USR-B-context-disagreement',
    modeClass: 'chat-only',
    variants: [
        {
            id: 'USR-B-context-disagreement',
            kind: 'user',
            categoryCode: 'B09',
            title: 'Voice a Disagreement',
            description: 'Voice a Disagreement',
...
Return ONLY the finished message in {{user_format}}, with no commentary. If there is no processable text, return \`[NO_TEXT_PROVIDED]\`.`,
            parameters: [
                {
                    ...
                    control: 'textarea',
                    optional: false,
                    label: 'What you disagree with + your alternative',
                },
                {
                    name: 'context',
                    control: 'select',
            ...
            examples: {
                user_text: [
                    "don't think we should rewrite the service from scratch, too risky; I'd rather refactor incrementally",
                ],
            },
            keywords: ['disagreement', 'diplomatic', 'alternative', 'edge case', 'context'],
