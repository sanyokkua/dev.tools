...
- `src/common/prompts/catalog/` — TypeScript catalog modules (A–D domains); source of truth for all prompts
- `src/common/prompts/skills/` — TypeScript skill modules (13 authored skills with bundled file content)
- `src/common/prompts/generated/` — git-kept placeholder; `manifest.generated.ts` and `loaders.generated.ts` are regenerated by `npm run build:prompts` (git-ignored)
...
### Adding a Prompt or Skill

Invoke the `add-prompt` skill — it covers creating a TypeScript module under `src/common/prompts/catalog/`, running `npm run build:prompts`, and verifying the result. See also: `docs/howto/add-prompts.md`.
