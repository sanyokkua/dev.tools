...
- `content/prompts-collection/` — prompt + skill Markdown source files (canonical; edit these, not the generated JSON)
- `src/common/prompts/generated/` — `prompts-data.json` and `skills-data.json` generated by `npm run ingest:prompts` (do not edit manually)
- `scripts/ingest-prompts.mjs` — ingestion pipeline: reads `content/`, validates, writes `src/common/prompts/generated/`
...
Invoke the `add-prompt` skill — it covers creating a Markdown file under `content/prompts-collection/`, running `npm run ingest:prompts`, and verifying the result. See also: `docs/howto/add-prompts.md`.
